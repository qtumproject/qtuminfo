...
export default class OutputScript extends Script {
  ...
  toString() {
    ...
    if (['OP_CREATE', 'OP_CALL'].includes(chunks[chunks.length - 1])) {
      for (let i = 0; i < 3; ++i) {
        chunks[i] = OutputScript.parseNumberChunk(chunks[i])
      }
    }
    return chunks.join(' ')
  }
  ...
  get type() {
    for (const [Class, typeString] of [
      [PublicKeyOutputScript, types.PUBKEY_OUT],
      [PublicKeyHashOutputScript, types.PUBKEYHASH_OUT],
      [ScriptHashOutputScript, types.SCRIPTHASH_OUT],
      [MultisigOutputScript, types.MULTISIG_OUT],
      [DataOutputScript, types.DATA_OUT],
      [WitnessV0KeyHashOutputScript, types.WITNESS_V0_KEYHASH],
      [WitnessV0ScriptHashOut, types.WITNESS_V0_SCRIPTHASH],
      [EVMContractCreateScript, types.EVM_CONTRACT_CREATE],
      [EVMContractCreateBySenderScript, types.EVM_CONTRACT_CREATE_SENDER],
      [EVMContractCallScript, types.EVM_CONTRACT_CALL],
      [EVMContractCallBySenderScript, types.EVM_CONTRACT_CALL_SENDER],
      [ContractOutputScript, types.CONTRACT_OUT]
    ]) {
...
export class ScriptHashOutputScript extends OutputScript {
  constructor(chunks) {
    super(chunks)
    this.scriptHash = chunks[0].buffer
  }
...
export class EVMContractCreateScript extends OutputScript {
  constructor(chunks) {
    super(chunks)
    this.gasLimit = OutputScript.parseNumberChunk(chunks[1])
    this.gasPrice = OutputScript.parseNumberChunk(chunks[2])
    this.byteCode = chunks[3].buffer
  }

  static build({gasLimit, gasPrice, byteCode}) {
    return new EVMContractCreateScript([
      OutputScript.buildNumberChunk(4),
      OutputScript.buildNumberChunk(gasLimit),
      OutputScript.buildNumberChunk(gasPrice),
  ...
  static isValid(chunks) {
    return chunks.length === 5
      && OutputScript.parseNumberChunk(chunks[0]) === 4
  ...
  toString() {
    return [
      4,
      OutputScript.parseNumberChunk(this.gasLimit),
      OutputScript.parseNumberChunk(this.gasPrice),
...
export class EVMContractCreateBySenderScript extends OutputScript {
  constructor(chunks) {
    super(chunks)
    this.addressType = OutputScript.parseNumberChunk(chunks[0])
    this.addressData = chunks[1].buffer
    this.signature = chunks[2].buffer
    this.gasLimit = OutputScript.parseNumberChunk(chunks[5])
    this.gasPrice = OutputScript.parseNumberChunk(chunks[6])
  ...
  static build({addressType, addressData, signature, gasLimit, gasPrice, byteCode}) {
    return new EVMContractCreateBySenderScript([
      OutputScript.buildNumberChunk(addressType),
  ...
  static isValid(chunks) {
    return chunks.length === 9
      ...
      && OutputScript.parseNumberChunk(chunks[4]) === 4
...
export class ContractOutputScript extends OutputScript {
  ...
  static build(contract) {
    return new EVMContractCallBySenderScript([
      OutputScript.buildNumberChunk(0),
      OutputScript.buildNumberChunk(0),
      OutputScript.buildNumberChunk(0),
  ...
  static isValid(chunks) {
    return chunks.length === 6
      && OutputScript.parseNumberChunk(chunks[0]) === 0
      && chunks[5].code === Opcode.OP_CALL
  }
